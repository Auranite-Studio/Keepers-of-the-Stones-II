...
public class UpdateChecker {
	...
	// Приоритеты типов версий: чем меньше число, тем "старше" версия
	private static final Map<CheckingModVersionType.VersionType, Integer> VERSION_PRIORITY = Map.of(
			CheckingModVersionType.VersionType.RELEASE, 0,
			CheckingModVersionType.VersionType.RELEASE_CANDIDATE, 1,
			CheckingModVersionType.VersionType.PRE_RELEASE, 2,
			CheckingModVersionType.VersionType.BETA, 3,
			CheckingModVersionType.VersionType.CUSTOM_SUFFIX, 4,
			CheckingModVersionType.VersionType.UNKNOWN, 5
	);
	...
	public static void checkForUpdates(ServerPlayer player) {
		try {
			...
			Version current = new Version(CURRENT_VERSION, CheckingModVersionType.get());
			Version latest = null;

			for (VersionData data : versions) {
				if (!isMinecraftVersionSupported(data.gameVersions)) continue;

				CheckingModVersionType.VersionType type = getVersionType(data.versionNumber);
				Version version = new Version(data.versionNumber, type);

				// Не предлагать обновление до нерелизных версий, если текущая — RELEASE
				if (current.type == CheckingModVersionType.VersionType.RELEASE &&
						version.type != CheckingModVersionType.VersionType.RELEASE) {
					continue;
				}
	...
	static class Version implements Comparable<Version> {
		...
		private final CheckingModVersionType.VersionType type;

		public Version(String version, CheckingModVersionType.VersionType type) {
			this.raw = version;
			this.type = type;
		}
		...
		public int compareTo(Version o) {
			...
			// Если типы одинаковые, сравниваем номера версий
		...
		public CheckingModVersionType.VersionType getVersionType() {
			return type;
		}
	...
	private static CheckingModVersionType.VersionType getVersionType(String version) {
		if (Pattern.matches("^\\d+\\.\\d+\\.\\d+$", version)) {
			return CheckingModVersionType.VersionType.RELEASE;
		} else if (Pattern.matches("^\\d+\\.\\d+\\.\\d+-pre\\d+$", version)) {
			return CheckingModVersionType.VersionType.PRE_RELEASE;
		} else if (Pattern.matches("^\\d+\\.\\d+\\.\\d+-rc\\d+$", version)) {
			return CheckingModVersionType.VersionType.RELEASE_CANDIDATE;
		} else if (Pattern.matches("^\\d+\\.\\d+\\.\\d+\\.\\d+$", version)) {
			return CheckingModVersionType.VersionType.BETA;
		} else if (Pattern.matches("^\\d+\\.\\d+\\.\\d+-[a-zA-Z0-9]+.*$", version)) {
			return CheckingModVersionType.VersionType.CUSTOM_SUFFIX;
		} else {
			return CheckingModVersionType.VersionType.UNKNOWN;
		}
	}
