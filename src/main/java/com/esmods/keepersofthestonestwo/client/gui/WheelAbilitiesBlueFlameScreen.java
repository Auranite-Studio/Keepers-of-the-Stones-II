...
public class WheelAbilitiesBlueFlameScreen extends AbstractContainerScreen<WheelAbilitiesBlueFlameMenu> implements PowerModScreens.ScreenAccessor {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 83 && mouseX < leftPos + 107 && mouseY > topPos + 23 && mouseY < topPos + 47) {
			ClientTooltipComponent clienttooltipcomponent = ClientTooltipComponent.create((FormattedCharSequence) Component.translatable("gui.power.wheel_abilities_blue_flame.tooltip_blue_flamethrower_uses_20"));
		...
		if (mouseX > leftPos + 144 && mouseX < leftPos + 168 && mouseY > topPos + 83 && mouseY < topPos + 107) {
			ClientTooltipComponent clienttooltipcomponent = ClientTooltipComponent.create((FormattedCharSequence) Component.translatable("gui.power.wheel_abilities_blue_flame.tooltip_sixshot_fireballs_uses_55"));
			guiGraphics.renderTooltip(font, List.of(clienttooltipcomponent), mouseX, mouseY, DefaultTooltipPositioner.INSTANCE, null);
			customTooltipShown = true;
		}
		if (mouseX > leftPos + 83 && mouseX < leftPos + 107 && mouseY > topPos + 145 && mouseY < topPos + 169) {
			ClientTooltipComponent clienttooltipcomponent = ClientTooltipComponent.create((FormattedCharSequence) Component.translatable("gui.power.wheel_abilities_blue_flame.tooltip_electric_discharge_uses_75"));
			guiGraphics.renderTooltip(font, List.of(clienttooltipcomponent), mouseX, mouseY, DefaultTooltipPositioner.INSTANCE, null);
			customTooltipShown = true;
		}
	...
	public void init() {
		...
		imagebutton_electric_discharge = new ImageButton(this.leftPos + 72, this.topPos + 134, 46, 46,
				new WidgetSprites(ResourceLocation.parse("power:textures/screens/electric_discharge.png"), ResourceLocation.parse("power:textures/screens/electric_discharge_highlight.png")), e -> {
					if (PowerLockCheckProcedure.execute(entity)) {
						PacketDistributor.sendToServer(new WheelAbilitiesBlueFlameButtonMessage(9, x, y, z));
