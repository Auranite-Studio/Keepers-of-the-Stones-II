...
public class WheelAbilitiesSoundScreen extends AbstractContainerScreen<WheelAbilitiesSoundMenu> implements PowerModScreens.ScreenAccessor {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 83 && mouseX < leftPos + 107 && mouseY > topPos + 22 && mouseY < topPos + 46) {
			ClientTooltipComponent clienttooltipcomponent = ClientTooltipComponent.create((FormattedCharSequence) Component.translatable("gui.power.wheel_abilities_sound.tooltip_sound_wave_uses_10"));
		...
		if (mouseX > leftPos + 144 && mouseX < leftPos + 168 && mouseY > topPos + 83 && mouseY < topPos + 107) {
			ClientTooltipComponent clienttooltipcomponent = ClientTooltipComponent.create((FormattedCharSequence) Component.translatable("gui.power.wheel_abilities_sound.tooltip_sound_bomb_uses_30"));
			guiGraphics.renderTooltip(font, List.of(clienttooltipcomponent), mouseX, mouseY, DefaultTooltipPositioner.INSTANCE, null);
			customTooltipShown = true;
		}
		if (mouseX > leftPos + 83 && mouseX < leftPos + 107 && mouseY > topPos + 146 && mouseY < topPos + 170) {
			ClientTooltipComponent clienttooltipcomponent = ClientTooltipComponent.create((FormattedCharSequence) Component.translatable("gui.power.wheel_abilities_sound.tooltip_sound_boom_uses_80"));
			guiGraphics.renderTooltip(font, List.of(clienttooltipcomponent), mouseX, mouseY, DefaultTooltipPositioner.INSTANCE, null);
			customTooltipShown = true;
		}
	...
	public void init() {
		...
		imagebutton_sound_boom = new ImageButton(this.leftPos + 72, this.topPos + 134, 46, 46,
				new WidgetSprites(ResourceLocation.parse("power:textures/screens/sound_boom.png"), ResourceLocation.parse("power:textures/screens/sound_boom_highlight.png")), e -> {
					if (PowerLockCheckProcedure.execute(entity)) {
						PacketDistributor.sendToServer(new WheelAbilitiesSoundButtonMessage(9, x, y, z));
