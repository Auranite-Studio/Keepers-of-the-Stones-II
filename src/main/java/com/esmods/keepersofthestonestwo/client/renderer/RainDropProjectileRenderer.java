...
public class RainDropProjectileRenderer extends EntityRenderer<RainDropProjectileEntity> {
	private static final ResourceLocation texture = ResourceLocation.parse("power:textures/entities/rain_drop.png");
	private final Modelrain_drop model;

	public RainDropProjectileRenderer(EntityRendererProvider.Context context) {
		super(context);
		model = new Modelrain_drop(context.bakeLayer(Modelrain_drop.LAYER_LOCATION));
	}

	@Override
	public void render(RainDropProjectileEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
	...
	@Override
	public ResourceLocation getTextureLocation(RainDropProjectileEntity entity) {
		return texture;
	}
}