...
public class AtomicRocketRenderer extends EntityRenderer<AtomicRocketEntity> {
	...
	public AtomicRocketRenderer(EntityRendererProvider.Context context) {
		super(context);
		model = new Modelatomic_rocket(context.bakeLayer(Modelatomic_rocket.LAYER_LOCATION));
	}

	@Override
	public void render(AtomicRocketEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
	...
	@Override
	public ResourceLocation getTextureLocation(AtomicRocketEntity entity) {
		return texture;
	}
}
