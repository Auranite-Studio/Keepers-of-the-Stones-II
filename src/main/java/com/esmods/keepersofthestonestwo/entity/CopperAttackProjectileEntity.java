...
import com.esmods.keepersofthestonestwo.procedures.CopperAttackProjectileKoghdaSnariadPopadaietVBlokProcedure;
...
public class CopperAttackProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public CopperAttackProjectileEntity(EntityType<? extends CopperAttackProjectileEntity> type, double x, double y, double z, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public CopperAttackProjectileEntity(EntityType<? extends CopperAttackProjectileEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		CopperAttackProjectileKoghdaSnariadPopadaietVBlokProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		CopperAttackProjectileKoghdaSnariadPopadaietVBlokProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}
	...
	public static CopperAttackProjectileEntity shoot(Level world, LivingEntity entity, RandomSource source) {
	...
	public static CopperAttackProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		CopperAttackProjectileEntity entityarrow = new CopperAttackProjectileEntity(PowerModEntities.COPPER_ATTACK_PROJECTILE.get(), entity, world, null);
	...
	public static CopperAttackProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		CopperAttackProjectileEntity entityarrow = new CopperAttackProjectileEntity(PowerModEntities.COPPER_ATTACK_PROJECTILE.get(), entity, entity.level(), null);
