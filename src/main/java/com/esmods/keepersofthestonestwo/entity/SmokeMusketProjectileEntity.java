...
public class SmokeMusketProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public SmokeMusketProjectileEntity(EntityType<? extends SmokeMusketProjectileEntity> type, double x, double y, double z, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public SmokeMusketProjectileEntity(EntityType<? extends SmokeMusketProjectileEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public static SmokeMusketProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		SmokeMusketProjectileEntity entityarrow = new SmokeMusketProjectileEntity(PowerModEntities.SMOKE_MUSKET_PROJECTILE.get(), entity, world, null);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("power:musket")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static SmokeMusketProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		SmokeMusketProjectileEntity entityarrow = new SmokeMusketProjectileEntity(PowerModEntities.SMOKE_MUSKET_PROJECTILE.get(), entity, entity.level(), null);
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("power:musket")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
