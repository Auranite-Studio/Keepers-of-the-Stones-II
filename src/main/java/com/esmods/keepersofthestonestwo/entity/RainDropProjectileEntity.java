...
public class RainDropProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public RainDropProjectileEntity(EntityType<? extends RainDropProjectileEntity> type, double x, double y, double z, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public RainDropProjectileEntity(EntityType<? extends RainDropProjectileEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public static RainDropProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		RainDropProjectileEntity entityarrow = new RainDropProjectileEntity(PowerModEntities.RAIN_DROP_PROJECTILE.get(), entity, world, null);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("block.bubble_column.bubble_pop")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static RainDropProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		RainDropProjectileEntity entityarrow = new RainDropProjectileEntity(PowerModEntities.RAIN_DROP_PROJECTILE.get(), entity, entity.level(), null);
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("block.bubble_column.bubble_pop")), SoundSource.PLAYERS, 1,
