...
public class BallLightningProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public BallLightningProjectileEntity(EntityType<? extends BallLightningProjectileEntity> type, double x, double y, double z, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public BallLightningProjectileEntity(EntityType<? extends BallLightningProjectileEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		BallLightningProjectileKoghdaSnariadPopadaietVSushchnostProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), entityHitResult.getEntity());
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		BallLightningProjectileKoghdaSnariadPopadaietVBlokProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}

	@Override
	public void tick() {
		super.tick();
		BallLightningProjectileKazhdyiTikPriPoliotieSnariadaProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
		if (this.isInGround())
			this.discard();
	}
	...
	public static BallLightningProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		BallLightningProjectileEntity entityarrow = new BallLightningProjectileEntity(PowerModEntities.BALL_LIGHTNING_PROJECTILE.get(), entity, world, null);
	...
	public static BallLightningProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		BallLightningProjectileEntity entityarrow = new BallLightningProjectileEntity(PowerModEntities.BALL_LIGHTNING_PROJECTILE.get(), entity, entity.level(), null);
