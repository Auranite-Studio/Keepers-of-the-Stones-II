...
public class ShadowSphereEntity extends AbstractArrow implements ItemSupplier {
	...
	public ShadowSphereEntity(EntityType<? extends ShadowSphereEntity> type, double x, double y, double z, Level world, @Nullable ItemStack firedFromWeapon) {
		super(type, x, y, z, world, PROJECTILE_ITEM, firedFromWeapon);
	}

	public ShadowSphereEntity(EntityType<? extends ShadowSphereEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		LightballProjectileKoghdaSnariadPopadaietVSushchnostProcedure.execute(entityHitResult.getEntity());
	}

	@Override
	public void tick() {
		super.tick();
		ShadowSphereKazhdyiTikPriPoliotieSnariadaProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
		if (this.isInGround())
			this.discard();
	}

	public static ShadowSphereEntity shoot(Level world, LivingEntity entity, RandomSource source) {
		return shoot(world, entity, source, 1f, 9, 2);
	}

	public static ShadowSphereEntity shoot(Level world, LivingEntity entity, RandomSource source, float pullingPower) {
	...
	public static ShadowSphereEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		ShadowSphereEntity entityarrow = new ShadowSphereEntity(PowerModEntities.SHADOW_SPHERE.get(), entity, world, null);
	...
	public static ShadowSphereEntity shoot(LivingEntity entity, LivingEntity target) {
		ShadowSphereEntity entityarrow = new ShadowSphereEntity(PowerModEntities.SHADOW_SPHERE.get(), entity, entity.level(), null);
