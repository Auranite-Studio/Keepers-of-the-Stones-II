			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 20) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("power:shockwave_impact")), SoundSource.PLAYERS, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("power:shockwave_impact")), SoundSource.PLAYERS, 1, 1, false);
					}
				}
				{
