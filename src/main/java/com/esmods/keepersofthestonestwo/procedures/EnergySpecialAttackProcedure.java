...
public class EnergySpecialAttackProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("energy_ability_1")) {
						...
						for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(1.3 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList()) {
							if (!(entityiterator == entity)) {
								...
								if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
									_entity.addEffect(new MobEffectInstance(PowerModMobEffects.POWER_LOCK, 60, 0));
							}
						}
					}
				}
		...
		} else if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("energy_ability_2")) {
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 35) {
				{
					Entity _shootFrom = entity;
					Level projectileLevel = _shootFrom.level();
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new EnergyChargeEntity(PowerModEntities.ENERGY_CHARGE.get(), projectileLevel), entity, (float) entity.getData(PowerModVariables.PLAYER_VARIABLES).base_damage_by_lvl, true, false,
		...
		} else if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("energy_ability_3")) {
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 80) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, 300, 6, false, false));
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("power:electric_spark")), SoundSource.PLAYERS, (float) 0.1, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("power:electric_spark")), SoundSource.PLAYERS, (float) 0.1, 1, false);
					}
				}
				particleAmount = 8;
				particleRadius = 2;
