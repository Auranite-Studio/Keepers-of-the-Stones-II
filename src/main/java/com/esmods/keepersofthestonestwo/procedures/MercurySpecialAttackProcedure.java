			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 35) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new MercuryBallProjectileEntity(PowerModEntities.MERCURY_BALL_PROJECTILE.get(), 0, 0, 0, projectileLevel, createArrowWeaponItemStack(projectileLevel, 2, (byte) 0)), entity,
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 55) {
				if (world.getBiome(BlockPos.containing(x, y, z)).value().getBaseTemperature() * 100f <= 0.15) {
					...
					if (world instanceof ServerLevel projectileLevel) {
						Projectile _entityToSpawn = initArrowProjectile(new MercuryBallProjectileEntity(PowerModEntities.MERCURY_BALL_PROJECTILE.get(), projectileLevel), entity,
								(float) (entity.getData(PowerModVariables.PLAYER_VARIABLES).base_damage_by_lvl * 1.8), true, false, false, AbstractArrow.Pickup.DISALLOWED);
