...
public class CursedKeeperShieldTickProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity instanceof CursedKeeperEntity) {
			if (entity.getPersistentData().getDoubleOr("windShield", 0) > 0) {
				if (timer > 40) {
					timer = timer - 1;
				} else if (timer == 0) {
					{
					...
					timer = 40;
					entity.getPersistentData().putDouble("windShield", (entity.getPersistentData().getDoubleOr("windShield", 0) - 1));
				}
			}
		}
	}
