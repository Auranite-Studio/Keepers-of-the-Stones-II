...
public class ShadowSpecialAttackProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		double Scaling = 0;
		if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("shadow_ability_1")) {
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 35) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new ShadowSphereEntity(PowerModEntities.SHADOW_SPHERE.get(), projectileLevel), entity, (float) (entity.getData(PowerModVariables.PLAYER_VARIABLES).base_damage_by_lvl * 1.4),
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 75) {
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = PowerModEntities.SHADOW.get().spawn(_level, BlockPos.containing(x, y + 1, z), EntitySpawnReason.MOB_SUMMONED);
