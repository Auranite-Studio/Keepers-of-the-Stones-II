		...
		if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("technology_ability_1")) {
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 15) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new TurretProjectileEntity(PowerModEntities.TURRET_PROJECTILE.get(), 0, 0, 0, projectileLevel, createArrowWeaponItemStack(projectileLevel, 2, (byte) 0)), entity,
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("entity.firework_rocket.shoot")), SoundSource.PLAYERS, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("entity.firework_rocket.shoot")), SoundSource.PLAYERS, 1, 1, false);
					}
				}
				{
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 80) {
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = PowerModEntities.TURRET.get().spawn(_level, BlockPos.containing(x, y + 1, z), EntitySpawnReason.MOB_SUMMONED);
