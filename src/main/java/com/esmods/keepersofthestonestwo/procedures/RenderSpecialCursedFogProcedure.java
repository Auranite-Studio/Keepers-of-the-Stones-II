...
public class RenderSpecialCursedFogProcedure {
	...
	private static void execute(@Nullable Event event, Entity entity) {
		if (entity == null)
			return;
		if (!entity.isUnderWater() && !(entity instanceof LivingEntity _livEnt1 && _livEnt1.hasEffect(PowerModMobEffects.MIST)) && entity.getData(PowerModVariables.PLAYER_VARIABLES).is_render_fog) {
			setDistance((float) entity.getData(PowerModVariables.PLAYER_VARIABLES).start_fog, (float) entity.getData(PowerModVariables.PLAYER_VARIABLES).end_fog);
