...
public class SpaceSpecialAttackProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		double Scaling = 0;
		if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("space_ability_1")) {
		...
		} else if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("space_ability_2")) {
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 45) {
				{
					Entity _shootFrom = entity;
					Level projectileLevel = _shootFrom.level();
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new MeteoriteProjectileEntity(PowerModEntities.METEORITE_PROJECTILE.get(), 0, 0, 0, projectileLevel, createArrowWeaponItemStack(projectileLevel, 8, (byte) 0)), entity,
