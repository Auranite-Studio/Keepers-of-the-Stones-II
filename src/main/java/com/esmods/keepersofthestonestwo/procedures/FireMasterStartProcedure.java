	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		for (int index0 = 0; index0 < 40; index0++) {
			if (world instanceof ServerLevel _level && _level.theGame() != null) {
				Optional<CommandFunction<CommandSourceStack>> _fopt = _level.theGame().getFunctions().get(ResourceLocation.parse("power:fire_master"));
				if (_fopt.isPresent())
		...
		if (entity.getData(PowerModVariables.PLAYER_VARIABLES).mergers == 0) {
			{
				...
				if (_entity instanceof LivingEntity _living) {
					_living.setItemSlot(EquipmentSlot.HEAD, new ItemStack(PowerModItems.FIRE_ARMOR_HELMET.get()));
				}
			}
			{
				Entity _entity = entity;
				if (_entity instanceof LivingEntity _living) {
					_living.setItemSlot(EquipmentSlot.CHEST, new ItemStack(PowerModItems.FIRE_ARMOR_CHESTPLATE.get()));
				}
			}
			{
				Entity _entity = entity;
				if (_entity instanceof LivingEntity _living) {
					_living.setItemSlot(EquipmentSlot.LEGS, new ItemStack(PowerModItems.FIRE_ARMOR_LEGGINGS.get()));
				}
			}
			{
				Entity _entity = entity;
			...
			{
				PowerModVariables.PlayerVariables _vars = entity.getData(PowerModVariables.PLAYER_VARIABLES);
				_vars.element_name_second = "fire";
				_vars.syncPlayerVariables(entity);
			}
