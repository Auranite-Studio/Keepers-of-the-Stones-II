			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 10) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("entity.warden.sonic_boom")), SoundSource.PLAYERS, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("entity.warden.sonic_boom")), SoundSource.PLAYERS, 1, 1, false);
					}
				}
				{
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 30) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new SoundBombProjectileEntity(PowerModEntities.SOUND_BOMB_PROJECTILE.get(), projectileLevel), entity,
								(float) (entity.getData(PowerModVariables.PLAYER_VARIABLES).base_damage_by_lvl * 1.45), true, false, false, AbstractArrow.Pickup.DISALLOWED);
