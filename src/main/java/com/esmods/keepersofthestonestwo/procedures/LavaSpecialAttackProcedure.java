			...
			if ((entity.getCapability(PowerModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new PowerModVariables.PlayerVariables())).power >= 35) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						...
						}.getArrow(projectileLevel, entity, (float) ((entity.getCapability(PowerModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new PowerModVariables.PlayerVariables())).base_damage_by_lvl * 1.34), 2, (byte) 2);
								...
								for (int zi = -horizontalRadiusHemiBot; zi <= horizontalRadiusHemiBot; zi++) {
									...
									if (distanceSq <= 1.0) {
										if (world.getBlockState(BlockPos.containing(x + xi, y + i - 1, z + zi)).canOcclude()
												&& !(world.getBlockState(BlockPos.containing(x + xi, y + i - 1, z + zi))).is(BlockTags.create(new ResourceLocation("c:unbreakable")))) {
