	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("vacuum_ability_1")) {
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 20) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new SphereNothingProjectileEntity(PowerModEntities.SPHERE_NOTHING_PROJECTILE.get(), 0, 0, 0, projectileLevel, createArrowWeaponItemStack(projectileLevel, 1, (byte) 1)), entity,
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("block.sculk.spread")), SoundSource.PLAYERS, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("block.sculk.spread")), SoundSource.PLAYERS, 1, 1, false);
					}
				}
				{
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 75) {
				...
				{
					Entity _ent = entity;
					_ent.teleportTo(((entity instanceof ServerPlayer _player && !_player.level().isClientSide())
							? ((_player.getRespawnConfig().dimension().equals(_player.level().dimension()) && _player.getRespawnConfig().pos() != null) ? _player.getRespawnConfig().pos().getX() : _player.level().getLevelData().getSpawnPos().getX())
							: 0),
					...
					if (_ent instanceof ServerPlayer _serverPlayer)
						_serverPlayer.connection.teleport(
								((entity instanceof ServerPlayer _player && !_player.level().isClientSide())
										? ((_player.getRespawnConfig().dimension().equals(_player.level().dimension()) && _player.getRespawnConfig().pos() != null)
												? _player.getRespawnConfig().pos().getX()
