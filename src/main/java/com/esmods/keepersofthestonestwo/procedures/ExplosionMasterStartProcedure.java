...
public class ExplosionMasterStartProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		for (int index0 = 0; index0 < 40; index0++) {
			if (world instanceof ServerLevel _level && _level.theGame() != null) {
				Optional<CommandFunction<CommandSourceStack>> _fopt = _level.theGame().getFunctions().get(ResourceLocation.parse("power:explosion_master"));
				if (_fopt.isPresent())
		...
		if (entity.getData(PowerModVariables.PLAYER_VARIABLES).mergers == 0) {
			...
			{
				PowerModVariables.PlayerVariables _vars = entity.getData(PowerModVariables.PLAYER_VARIABLES);
				_vars.element_name_first = "explosion";
				_vars.syncPlayerVariables(entity);
			}
			...
			{
				PowerModVariables.PlayerVariables _vars = entity.getData(PowerModVariables.PLAYER_VARIABLES);
				_vars.element_name_second = "explosion";
				_vars.syncPlayerVariables(entity);
			}
			...
			{
				PowerModVariables.PlayerVariables _vars = entity.getData(PowerModVariables.PLAYER_VARIABLES);
				_vars.element_name_third = "explosion";
				_vars.syncPlayerVariables(entity);
			}
		}
		if (entity instanceof LivingEntity _entity) {
			ItemStack _setstack = new ItemStack(PowerModItems.EXPLOSION_MACE.get()).copy();
