			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 20) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new GrenadeEntity(PowerModEntities.GRENADE.get(), projectileLevel), entity, (float) entity.getData(PowerModVariables.PLAYER_VARIABLES).base_damage_by_lvl, true, false, false,
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("block.copper.fall")), SoundSource.PLAYERS, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("block.copper.fall")), SoundSource.PLAYERS, 1, 1, false);
					}
				}
				{
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 100) {
				{
					...
					Level projectileLevel = _shootFrom.level();
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new AtomicRocketEntity(PowerModEntities.ATOMIC_ROCKET.get(), 0, 0, 0, projectileLevel, createArrowWeaponItemStack(projectileLevel, 10, (byte) 10)), entity,
								(float) (entity.getData(PowerModVariables.PLAYER_VARIABLES).base_damage_by_lvl * 3.1), true, false, false, AbstractArrow.Pickup.DISALLOWED);
						_entityToSpawn.setPos(_shootFrom.getX(), _shootFrom.getEyeY() - 0.1, _shootFrom.getZ());
