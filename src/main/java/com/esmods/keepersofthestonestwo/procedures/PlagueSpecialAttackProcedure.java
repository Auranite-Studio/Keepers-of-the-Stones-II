			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 15) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						...
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("entity.wither.shoot")), SoundSource.PLAYERS, 1, 1, false);
					}
				}
				{
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 40) {
				{
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = initArrowProjectile(new PlagueBombEntity(PowerModEntities.PLAGUE_BOMB.get(), 0, 0, 0, projectileLevel, createArrowWeaponItemStack(projectileLevel, 4, (byte) 0)), entity,
