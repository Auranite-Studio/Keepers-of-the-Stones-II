...
public class QuakePowerProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, CommandContext<CommandSourceStack> arguments) {
		if (world instanceof ServerLevel _level)
			_level.theGame().getCommands()
					.performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.theGame(), null).withSuppressedOutput(),
		...
		PowerMod.queueServerWork(1, () -> {
			if (world instanceof ServerLevel _level)
				_level.theGame().getCommands().performPrefixedCommand(
						new CommandSourceStack(CommandSource.NULL, new Vec3((x + DoubleArgumentType.getDouble(arguments, "x_translate")), y, (z + DoubleArgumentType.getDouble(arguments, "z_translate"))), Vec2.ZERO, _level, 4, "",
								Component.literal(""), _level.theGame(), null).withSuppressedOutput(),
					...
					if (world instanceof ServerLevel _level)
						_level.theGame().getCommands()
								.performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3((x + DoubleArgumentType.getDouble(arguments, "x_translate")), y, (z + DoubleArgumentType.getDouble(arguments, "z_translate"))), Vec2.ZERO,
										_level, 4, "", Component.literal(""), _level.theGame(), null).withSuppressedOutput(), "kill @e[type=block_display,sort=nearest,limit=1]");
				});
			});
		});
	}
