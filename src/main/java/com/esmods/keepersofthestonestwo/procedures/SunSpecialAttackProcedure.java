...
public class SunSpecialAttackProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("sun_ability_1")) {
								...
								}
								{
									...
									if (!_ent.level().isClientSide() && _ent.theGame() != null) {
										_ent.theGame().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null,
												4, _ent.getName().getString(), _ent.getDisplayName(), _ent.level().theGame(), _ent), "particle power:sun_particles ~ ~10 ~ 0.5 10 0.5 10 500 force");
									}
								}
								success = true;
							}
		...
		} else if ((entity.getData(PowerModVariables.PLAYER_VARIABLES).ability).equals("sun_ability_2")) {
			...
			if (entity.getData(PowerModVariables.PLAYER_VARIABLES).power >= 80 && entity.isShiftKeyDown() == true) {
				particleAmount = 125;
				particleRadius = 5;
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
