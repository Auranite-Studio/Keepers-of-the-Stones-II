...
public record WheelAbilitiesSoundButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	public static final Type<WheelAbilitiesSoundButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PowerMod.MODID, "wheel_abilities_sound_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesSoundButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesSoundButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesSoundButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesSoundButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesSoundButtonMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesSoundButtonMessage.TYPE, WheelAbilitiesSoundButtonMessage.STREAM_CODEC, WheelAbilitiesSoundButtonMessage::handleData);
	}
}