...
import com.esmods.keepersofthestonestwo.procedures.Attack191Procedure;
import com.esmods.keepersofthestonestwo.procedures.Attack190Procedure;
...
public record WheelAbilityDarknessButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilityDarknessButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilityDarknessButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilityDarknessButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilityDarknessButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilityDarknessButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = WheelAbilityDarknessMenu.guistate;
		...
		if (buttonID == 7) {
			...
			Attack189Procedure.execute(entity);
		}
		if (buttonID == 8) {

			Attack190Procedure.execute(entity);
		}
		if (buttonID == 9) {

			Attack191Procedure.execute(entity);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilityDarknessButtonMessage.TYPE, WheelAbilityDarknessButtonMessage.STREAM_CODEC, WheelAbilityDarknessButtonMessage::handleData);
	}
}
