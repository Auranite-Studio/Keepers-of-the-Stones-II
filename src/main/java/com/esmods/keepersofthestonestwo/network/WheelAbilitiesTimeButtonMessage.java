...
public record WheelAbilitiesTimeButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<WheelAbilitiesTimeButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PowerMod.MODID, "wheel_abilities_time_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesTimeButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesTimeButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesTimeButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesTimeButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesTimeButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = WheelAbilitiesTimeMenu.guistate;
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesTimeButtonMessage.TYPE, WheelAbilitiesTimeButtonMessage.STREAM_CODEC, WheelAbilitiesTimeButtonMessage::handleData);
	}
}
