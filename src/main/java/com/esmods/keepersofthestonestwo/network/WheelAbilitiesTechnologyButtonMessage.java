...
public record WheelAbilitiesTechnologyButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<WheelAbilitiesTechnologyButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PowerMod.MODID, "wheel_abilities_technology_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesTechnologyButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesTechnologyButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesTechnologyButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesTechnologyButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesTechnologyButtonMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesTechnologyButtonMessage.TYPE, WheelAbilitiesTechnologyButtonMessage.STREAM_CODEC, WheelAbilitiesTechnologyButtonMessage::handleData);
	}
}
