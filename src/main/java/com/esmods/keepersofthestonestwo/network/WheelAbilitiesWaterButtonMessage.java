...
public record WheelAbilitiesWaterButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesWaterButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesWaterButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesWaterButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesWaterButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesWaterButtonMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesWaterButtonMessage.TYPE, WheelAbilitiesWaterButtonMessage.STREAM_CODEC, WheelAbilitiesWaterButtonMessage::handleData);
	}
}
