...
public record WheelAbilitiesEnergyButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<WheelAbilitiesEnergyButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PowerMod.MODID, "wheel_abilities_energy_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesEnergyButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesEnergyButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesEnergyButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesEnergyButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesEnergyButtonMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesEnergyButtonMessage.TYPE, WheelAbilitiesEnergyButtonMessage.STREAM_CODEC, WheelAbilitiesEnergyButtonMessage::handleData);
	}
}