...
public record WheelAbilitiesTeleportationButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<WheelAbilitiesTeleportationButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PowerMod.MODID, "wheel_abilities_teleportation_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesTeleportationButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesTeleportationButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesTeleportationButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesTeleportationButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesTeleportationButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = WheelAbilitiesTeleportationMenu.guistate;
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesTeleportationButtonMessage.TYPE, WheelAbilitiesTeleportationButtonMessage.STREAM_CODEC, WheelAbilitiesTeleportationButtonMessage::handleData);
	}
}
