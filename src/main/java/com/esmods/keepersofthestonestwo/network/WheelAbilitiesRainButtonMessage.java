...
public record WheelAbilitiesRainButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<WheelAbilitiesRainButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PowerMod.MODID, "wheel_abilities_rain_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesRainButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesRainButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesRainButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesRainButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesRainButtonMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesRainButtonMessage.TYPE, WheelAbilitiesRainButtonMessage.STREAM_CODEC, WheelAbilitiesRainButtonMessage::handleData);
	}
}