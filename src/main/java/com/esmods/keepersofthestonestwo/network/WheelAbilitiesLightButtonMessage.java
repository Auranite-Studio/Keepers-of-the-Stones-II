...
public record WheelAbilitiesLightButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<WheelAbilitiesLightButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PowerMod.MODID, "wheel_abilities_light_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesLightButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesLightButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesLightButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesLightButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesLightButtonMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesLightButtonMessage.TYPE, WheelAbilitiesLightButtonMessage.STREAM_CODEC, WheelAbilitiesLightButtonMessage::handleData);
	}
}
