...
public record WheelAbilitiesPoisonButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<WheelAbilitiesPoisonButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PowerMod.MODID, "wheel_abilities_poison_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, WheelAbilitiesPoisonButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, WheelAbilitiesPoisonButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new WheelAbilitiesPoisonButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<WheelAbilitiesPoisonButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final WheelAbilitiesPoisonButtonMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PowerMod.addNetworkMessage(WheelAbilitiesPoisonButtonMessage.TYPE, WheelAbilitiesPoisonButtonMessage.STREAM_CODEC, WheelAbilitiesPoisonButtonMessage::handleData);
	}
}
